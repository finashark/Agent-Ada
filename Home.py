"""
Home.py - Trang chủ OPTIMIZED - Load siêu nhanh < 1s
"""
...
# ⚡ SPEED OPTIMIZATION
st.success("⚡ **Tối ưu tốc độ:** Trang load nhanh <1s! Data được lazy-load khi cần.")

st.markdown("---")

# Hiển thị thông tin phiên giao dịch
...
<div class="info-box">
    ...
    • <strong>4 phiên giao dịch/ngày:</strong> Asia, Europe, US, After-Hours<br>
    • <strong>Cache thông minh:</strong> Data được share giữa tất cả users<br>
    • <strong>Tự động refresh:</strong> Khi sang phiên mới (4 lần/ngày)<br>
    • <strong>Tiết kiệm API calls:</strong> User đầu tiên fetch, các user khác dùng cache
</div>
""", unsafe_allow_html=True)

st.markdown("""
<div class="info-box">
    <strong>⚡ Tối ưu tốc độ:</strong><br>
    • <strong>Lazy loading:</strong> Data chỉ load khi user cần<br>
    • <strong>Shared cache:</strong> Tiết kiệm bandwidth và thời gian<br>
    • <strong>Session-based:</strong> Chỉ 4 lần refresh/ngày<br>
    • <strong>Fast initial load:</strong> Trang chủ load <1s
</div>
""", unsafe_allow_html=True)
...
with st.expander("🚀 Bắt đầu nhanh", expanded=True):
    ...
    3. **Data tự động cache** - user đầu tiên trong phiên sẽ fetch data, các user khác dùng cache
...
with st.expander("📊 Hiểu về phiên giao dịch"):
    st.markdown("""
    Ứng dụng theo dõi 4 phiên giao dịch chính:
    
    - 🌏 **Asia (Singapore):** 09:00 - 16:30 (giờ địa phương)
    - 🇬🇧 **Europe (London):** 08:00 - 16:30 (giờ địa phương)
    - 🇺🇸 **US (New York):** 09:30 - 16:00 ET
    - 🌙 **After-Hours (NY):** 16:00 - 20:00 ET
    
    **Cache strategy:**
    - Mỗi phiên có 1 cache key riêng
    - Khi sang phiên mới, cache tự động invalidate
    - User đầu tiên trong phiên fetch data mới
    - Các user tiếp theo dùng cached data
    """)
